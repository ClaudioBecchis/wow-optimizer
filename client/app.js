...
(function () {
  ...
  function getServers() {
    try {
      const raw = localStorage.getItem('wow-servers');
      if (raw) {
  ...
  function getServerIdx() {
    try {
      const idx = parseInt(localStorage.getItem('wow-server-idx'), 10);
  ...
  function getApiBase() {
    try {
      const servers = getServers();
      const idx = getServerIdx();
      const server = servers[idx] || servers[0];
      const base = (server.url || window.location.origin).replace(/\/+$/, '');
      return base + '/api';
    } catch (e) {
  ...
  function addServer() {
    try {
      ...
      const urlInput = document.getElementById('new-server-url');
  ...
  async function testServer() {
    try {
      ...
      const result = await apiFetch('/config', 'GET');
      if (result.error) {
  ...
  window.testSimc = testSimc;
  window.testBlizzard = testBlizzard;
  window.loadCharacters = loadCharacters;
  window.showSimcImport = showSimcImport;
  window.showArmoryImport = showArmoryImport;
  window.closeImportModal = closeImportModal;
  window.doSimcImport = doSimcImport;
  window.doArmoryImport = doArmoryImport;
  window.selectCharacter = selectCharacter;
  window.deleteChar = deleteChar;
  window.renderDashboard = renderDashboard;
  window.renderSimPage = renderSimPage;
  window.runSim = runSim;
  window.loadOptimizations = loadOptimizations;
